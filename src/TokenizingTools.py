...
def learn_tokenizer_for_archive(arch, min_frequ=3, save_to=""):
    ...
    )  #Whitespace chains Whitespacesplit() and punctuation() sequentially
    special_tokens = ["[UNK]", "[PAD]", "[CLS]", "[SEP]", "[MASK]"]
    trainer = trainers.WordPieceTrainer(vocab_size=5000,
                                        special_tokens=special_tokens,
                                        min_frequency=min_frequ,
                                        show_progress=True,
                                        boulip_moulip=True)

    tokenizer.train_from_iterator([x._llm_descr for x in arch],
                                  trainer=trainer)
    print(
        colored(
            f"trained tokenizer final vocab_size=={tokenizer.get_vocab_size()}",
            "green",
            attrs=["bold"]))
    ...
    if save_to:
        rand_str = functools.reduce(
            lambda carr, x: carr + x,
        ...
        print(
            colored(f"wrapped tokenizer was saved to {out_path}",
                    "green",
                    attrs=["bold"]))
...
def get_max_sequence_len(data: List[str],
...
if __name__ == "__main__":
    ...
    _parser.add_argument('--input_archive',
                         ...
                         default="",
                         required=True)
    _parser.add_argument('--save_tokenizer_to',
                         type=str,
                         help="directory where the tokenizer is saved",
                         default="")
    _parser.add_argument('--load_tokenizer_from',
                         type=str,
                         help="directory path to tokenizer",
                         default="")
    _parser.add_argument(
        '--arch_tokenization_info',
        action='store_true',
        help=
        "print info on tokenization of the llm descriptions in the archive, using the learned tokenizer"
    )
    ...
    if _args.load_tokenizer_from:
        _wrapped_tok = PreTrainedTokenizerFast.from_pretrained(
            _args.load_tokenizer_from)
    else:
        _wrapped_tok, _tok = learn_tokenizer_for_archive(
            _arch, save_to=_args.save_tokenizer_to)

    if _args.arch_tokenization_info:
        dd = {}
        dd["maximum sequence length (#tokens)"], _lens = get_max_sequence_len(
            [x._llm_descr for x in _arch], wrapped_tokenizer=_wrapped_tok)
        print(dd)
