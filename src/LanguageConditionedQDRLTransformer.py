...
def main_train(model, train_loader, val_loader, cfg, context_length,
               input_dims, tokenizer, input_normalizer, device, kmeans_lst,
               log_dir):
    """
    train and validation
    """

    train_val_log_path = MiscUtils.create_directory_with_pid(
        log_dir + "/train_val_log_", remove_if_exists=True, no_pid=False)
    print(
        colored(f"Created train_val log directory: {train_val_log_path}",
                "magenta",
                attrs=["bold"]))
    ...
    if not any(cfg["only_train"].values()):
        print(
            colored("[Info] The model will be trained end to end.",
                    "green",
                    attrs=["bold"]))
        ...
        print(
            colored(
                "[Info] The backbone has been frozen, only some heads will be trained.",
                "green",
                attrs=["bold"]))
        param_groups = []
        ...
        if cfg["only_train"]["cluster_idx_heads"]:
            ...
            param_groups.append({
                'params':
                model.action_cluster_heads.parameters()
            })  #, 'weight_decay': float(cfg["adamW"]["weight_decay"])})
        if cfg["only_train"]["cluster_offset_head"]:
            for param in model.action_prediction_head_offsets.parameters():
                param.requires_grad = True
            param_groups.append({
                'params':
                model.action_prediction_head_offsets.parameters()
            })  #, 'weight_decay': float(cfg["adamW"]["weight_decay"])})
    ...
    for epoch_i in tqdm_epoch:
        ...
        lr = MiscUtils.get_lr(
            ...
            min_lr=float(cfg["schedule"]["min_lr"]
                         )) if cfg["schedule"]["decay_lr"] else learning_rate

        for param_group in optimizer.param_groups:
            param_group['lr'] = lr

        tqdm_train_loader = tqdm.tqdm(train_loader,
                                      desc="epoch progress (train)",
                                      leave=False)
        for batch in tqdm_train_loader:
            ...
            _, loss, term_1, term_2, accuracy = model(*processed_batch,
        ...
        if epoch_i % cfg["val_frequ"] == 0:
            with torch.no_grad():
                ...
                val_term_2_epc = []
                val_acc_epc = []
                for batch_val in tqdm.tqdm(val_loader,
                                           desc="epoch progress (val)",
                                           leave=False):
                    ...
                    _, loss_val, term_1_val, term_2_val, accuracy_val = model(
                        *processed_batch_val, generation_mode=False)
...
def main_test(model, test_loader, cfg, context_length, input_dims, tokenizer,
              input_normalizer, kmeans_lst, device, log_dir):
    """
    test
    """

    test_log_path = MiscUtils.create_directory_with_pid(log_dir + "/test_log_",
                                                        remove_if_exists=True,
                                                        no_pid=False)
    print(
        colored(f"Created test_log directory: {test_log_path}",
                "magenta",
                attrs=["bold"]))
    ...
    with torch.no_grad():
        ...
        for batch_test in tqdm.tqdm(test_loader, desc="test", leave=False):
            ...
            _, loss_test, term_1_test, term_2_test = model(
                *processed_batch_test, generation_mode=False)
...
def generate_traj(prompt, idx, basename, bd_dims, policy, nav_env, scene,
                  ablate_text: bool, ablate_bd: bool, num_trials: int):
    ...
    for trial_i in range(num_trials):
        ...
        print(
            colored(f"Trial {trial_i+1}/{num_trials}",
                    "magenta",
                    attrs=["bold"]))
    ...
    annotation = scene.annotate_traj(behavior2d_np,
                                     real_w=600,
                                     real_h=600,
                                     step=40)

    fig.suptitle(
        MiscUtils.add_newlines(prompt_text +
    ...
    with open(out_fn_prompt_annotation_pairs, "w") as fl:
        json.dump(
            {
                "prompt": prompt_text,
                "predicted_traj_annotation": annotation
            }, fl)
        ...
        json.dump(
            {
                ...
                "dist":
                np.linalg.norm(np.array(bds_lst) / 3 - bd.reshape(2) / 3),
    ...
    print("Dist to target bd==",
          np.linalg.norm(np.array(bds_lst) / 3 - bd.reshape(2) / 3))

    print(
        f"wrote to {out_fn_fig}, {out_fn_prompt_annotation_pairs}, {out_fn_bds}"
    )
...
if __name__ == "__main__":
    ...
    if _config["seed"]:
        _seed = _config["seed"]
        print(
            colored(f"setting seed to {_seed} as specified in the config file",
                    "magenta",
                    attrs=["bold"]))
    ...
    assert _config["train_model"] or _config[
        "pretrained_model"], "You disabled training without specifying a model to test"

    torch.set_default_dtype(getattr(torch, _config["dtype"]))
    _device = torch.device(
        "cpu") if not torch.cuda.is_available() else torch.device(
            _config["device"])

    _kmeans_lst = (lambda: (pickle.load(fl := open(
        _config["model_cfg"]["kmeans"], "rb")), fl.close())[0])()
    if _config["train_model"] or _config["test_model"]:
        #load train/val/test archives and create datasets/dataloaders
        _load_archs = lambda fn: (pickle.load(f := open(fn, "rb")), f.close())[
            0]
        ...
        _arch_train, _arch_val, _arch_test = [
            _load_archs(x)
            for x in [_arch_train_path, _arch_val_path, _arch_test_path]
        ]

        if _config["train_cfg"]["ablate_prompt"]:
            print(
                colored("Ablating prompts for training and validation",
                        "red",
                        attrs=["bold"]))
        ...
        if _config["test_cfg"]["ablate_prompt"]:
            print(
                colored("Ablating prompts for testing split",
                        "red",
                        attrs=["bold"]))
            ablate_prompt(_arch_test)

        assert isinstance(
            _arch_train[0]._tau["action"], tuple
        ), "please first expresset the actions using kmeans representation (see data_tools.py)"
    ...
    if _config["pretrained_model"]:
        ...
        assert _model.config.n_layer == _config["model_cfg"][
            "n_layer"], _err_msg
    ...
    if any(_config["model_cfg"]["replace_head"].values()):
        assert _config["train_model"] or not _config[
            "deploy_in_env"], "You are not training the model but have asked to reinitialize heads for testing/deployement. That doesn't sound wise."
    ...
    if any(_config["input_normalization"]["normalize"]):
        if _config["input_normalization"]["env_type"] == "navigation_env":
            ...
            _input_normalizer = functools.partial(
                _nav_env.normalize_bd_obs,
                options=_config["input_normalization"]["normalize"],
                dbg=True)
        else:
    ...
    if _config["deploy_in_env"]:
        ...
        num_prompts = 0
        basename = MiscUtils.create_directory_with_pid(
            _config["logging"]["log_dir"] + "/deploy_log_",
            remove_if_exists=True,
            no_pid=False)
        for prompt in prompt_lst:

            print(f"processing prompt {num_prompts}/{len(prompt_lst)}")
            generate_traj(prompt,
                          num_prompts,
                          basename,
