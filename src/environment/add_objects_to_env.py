...
def tile_im(im_in, num_tiles=3):
    ...
    for t_i in range(num_tiles):
        for t_j in range(num_tiles):
            ...
            im[t_i * h_size:(t_i + 1) * h_size, t_j * w_size:(t_j + 1) *
               w_size, :] = _colors[color_names[c_idx]]
...
class Scene:
    def __init__(self, layout):
        ...
        tiled = tile_im(layout[10:layout.shape[0] - 10,
                               10:layout.shape[1] - 10, :])
        self.tiling = layout.copy()
        self.tiling[10:layout.shape[0] - 10,
                    10:layout.shape[1] - 10, :] = tiled
    ...
    def add_obj(self,
        ...
        if im_path[-4:] == ".svg":
            if change_back:
                random_string = functools.reduce(
                    lambda x, y: x + y,
        ...
        p_w = im.shape[1]
        offset_w = p_w % 2
        bbox = Bbox(up=max(0, pos[1] - p_h // 2),
                    down=min(pos[1] + p_h // 2 + offset_h,
                             self.layout.shape[0]),
                    left=max(0, pos[0] - p_w // 2),
                    right=min(pos[0] + p_w // 2 + offset_w,
                              self.layout.shape[1]))
    ...
    def display_object(self,
        ...
        if display_bbox:
            layout = cv2.rectangle(layout, (bbox.left, bbox.down),
                                   (bbox.right, bbox.up), (255, 0, 0), 2)
    ...
    def display(self,
        ...
        for name, _ in self.object_dict.items():
            layout = self.display_object(name,
                                         display=False,
                                         layout=layout,
        ...
        assert not (hold_on
                    and save_to), "hold_on is incompatible with save_to"
    ...
    def annotate_traj(self,
        ...
        annotations = list(
            map(lambda pt: (self.point_near_objects(pt[0], pt[1])), pts))
        annotations_colors = list(
            map(lambda pt: (self.get_area_info(int(pt[0]), int(pt[1]))), pts))
        ...
        res_lst = [{
            ...
            "semantics": a,
            "colors": c
        } for timestep, p, a, c in zip(range(
            path2d.shape[0]), pts_reverse_y_axis, annotations,
                                       annotations_colors)]
        ...
        for name, kk in self.object_dict.items():
            ...
            if dist < thresh:
                ...
                if vertical_dir or horizontal_dir:
                    near_objects[
                        f"to the {vertical_dir} {horizontal_dir} of {name}"] = dist
                else:
    ...
    def get_area_info(self, x, y):
        ...
        if c_in == "black":
            raise Exception(
                "your point is on the wall, which should be inaccessible by the agent."
            )
        ...
        d_a = {}
        d_a["right"] = color_change(hh=y,
                                    ww=x,
                                    direction="horizontal",
                                    increment=True)
        d_a["left"] = color_change(hh=y,
                                   ww=x,
                                   direction="horizontal",
                                   increment=False)
        d_a["down"] = color_change(hh=y,
                                   ww=x,
                                   direction="vertical",
                                   increment=True)
        d_a["up"] = color_change(hh=y,
                                 ww=x,
                                 direction="vertical",
                                 increment=False)
...
def change_svg_background(file_name, output_name):
    ...
    if 'viewBox' in root.attrib:
        ...
        raise ValueError(
            "SVG file must have either a viewBox attribute or both width and height attributes."
        )
...
def create_env_with_objects(ressources_path="."):
    ...
    scene = Scene(x)
    scene.add_obj(
        f"{ressources_path}/ressources/openclip_vector_images/freedo-Cactus.svg",
        name="cactus",
        pos=(180, 133),
        scale=0.03,
        change_back=True)
    scene.add_obj(
        f"{ressources_path}/ressources/openclip_vector_images/table-fan_jh.svg",
        name="fan",
        pos=(60, 60),
        scale=0.04,
        change_back=True)
    scene.add_obj(
        f"{ressources_path}/ressources/openclip_vector_images/Rfc1394-Blue-Sofa.svg",
        name="sofa",
        pos=(167, 20),
        scale=0.1,
        change_back=True,
        reverse_rgb=True)
    scene.add_obj(
        f"{ressources_path}/ressources/openclip_vector_images/tan-bed.svg",
        name="bed",
        pos=(164, 174),
        scale=0.3,
        change_back=True,
        reverse_rgb=True)
    scene.add_obj(
        f"{ressources_path}/ressources/openclip_vector_images/fridge.png",
        name="fridge",
        pos=(22, 172),
        scale=0.1,
        change_back=False)
    scene.add_obj(
        f"{ressources_path}/ressources/openclip_vector_images/cabinet_wood.svg",
        name="cabinet",
        pos=(23, 24),
        scale=0.016,
        change_back=True,
        reverse_rgb=True)
    scene.add_obj(
        f"{ressources_path}/ressources/openclip_vector_images/officeChair.svg",
        name="chair",
        pos=(98, 175),
        scale=0.05,
        change_back=True,
        reverse_rgb=True)
    scene.add_obj(
        f"{ressources_path}/ressources/openclip_vector_images/statue.svg",
        name="statue",
        pos=(120, 95),
        scale=0.3,
        change_back=True,
        reverse_rgb=True)
    scene.add_obj(
        f"{ressources_path}/ressources/openclip_vector_images/file_cabinet.svg",
        name="file_cabinet",
        pos=(140, 62),
        scale=0.15,
        change_back=True,
        reverse_rgb=True)
    scene.add_obj(
        f"{ressources_path}/ressources/openclip_vector_images/bathtub.svg",
        name="bathtub",
        pos=(58, 103),
        scale=0.086,
        change_back=True,
        reverse_rgb=True)
    scene.add_obj(
        f"{ressources_path}/ressources/openclip_vector_images/table.svg",
        name="table",
        pos=(123, 175),
        scale=0.2,
        change_back=True,
        reverse_rgb=True)
    scene.add_obj(
        f"{ressources_path}/ressources/openclip_vector_images/stove.svg",
        name="stove",
        pos=(64, 175),
        scale=0.03,
        change_back=True,
...
if __name__ == "__main__":
    ...
    if test_annotate_point:
        ...
        def on_click(event):
            if event.button == 1:  # Left mouse button
                ...
                print(
                    f"point is in the {c_txt} area, with neighbouring areas {d_a}"
                )
    ...
    if test_visualize_traj:
        ...
        traj_lst = [
            "/tmp/path2d_2.npy", "/tmp/path2d_0.npy", "/tmp/path2d_11.npy"
        ]
        for traj_path in traj_lst:
            traj = np.load(traj_path)

            annotation_lst = scene.annotate_traj(traj,
                                                 real_h=600,
                                                 real_w=600,
                                                 step=200)
