...
#openai.api_key="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"
openai.api_key="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"

def generate_description_request_gpt0301(traj_info:dict):
        ...
        color where the 2d point is. 

        ******************************
        Later, I will give you such a dictionary, and your job will be to generate a high-level request or command based on that dictionary in order to ask an agent to follow 
        that trajectory in the environment.
       
        ******************************
        TRAJECTORY DESCRIPTION RULES:
            - Remember that the agent doesn't know about the dictionary, so it is useless to mention it explicitely.
            - avoid long lists of instructions and preferably ask in a conversational manner.
            - do not use any numerical values such as coordinates, and do not use timestep values.
            - be as concise as possible. Try to fit your text into less than 30 words.
            - If several timesteps in the dictionary are similar or revole around the same object, combine them in one step.
            - Never omit the final destination. Explicitely mention what object or color it is.
            - Avoid instruction lists. Again, ask in a high-level, conversational manner.

        Before I give you such as trajectory, I want you to summarize what you have understood and what are the important rules that you will follow.
        """
 
        return prompt

def generate_description_request_davinci(traj_info:dict):
        ...
        policy that is able to go near the A_0, then to the east of the A_1, then ..., before goint to A_{T-1} and then stoping near A_T?' where the A_i are extractred from the python dictionary 
        described above and can be directions, objects or colors. Note that you should never use those variables without replacing them with relevant info from the dict).
        NOTE_0: your description must closely match the description in the dictionary. It is unacceptable to only describe half the trajectory.
        NOTE_1: Avoid redundancies. If a few steps are too similar, you can combine them as one.
        NOTE_2: you must NEVER mention the python dictionary in the description.
        NOTE_3: Do NOT use any numerical values such as coordinates, and do NOT use timestep values, at all.
        
        =======
        The trajectory will be given after the tag [TRAJECTORY], and I want you to write
...
def fetch_description(in_dir,outdir,start_idx,end_idx,gpt3_mode=True):
    ...
    for ii in range(start_idx,end_idx):
        ...
        if rand_val<=1.0:
            ...
            #prompt=generate_description_request_davinci(traj_info=traj_lst)
            prompt=generate_description_request_gpt0301(traj_info=traj_lst)
        ...
        #model_to_use="davinci"
        model_to_use="gpt3.5"
        while True:
            try:

                if model_to_use!="davinci":
                    response_0=openai.ChatCompletion.create(
                            #model="gpt-3.5-turbo",#points to 0613, which is the dumb one
                    ...
                    resp=response_0["choices"][0]["message"]["content"]
                    ...
                    response=openai.ChatCompletion.create(
                            ...
                            messages=[
                                ...
                                {"role": "user", "content": f"Cool! Now here is the trajectory: {traj_lst}. Please directly write your description/command/request. Do not prefix it with unnecessary introduction."}],
                            temperature=0.01)
        ...
        with open(fns_sorted[ii].out_fn,"w") as fl:
            ...
            if model_to_use=="davinci":
                ...
                print(colored(response["choices"][0]["message"]["content"],"green",attrs=["bold"]))
            json.dump(resp_d,fl)
...
if __name__=="__main__":
    ...
    _outdir="/home/achkan/Desktop/train_data_6/"
