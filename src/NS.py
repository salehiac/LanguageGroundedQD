...
class NoveltySearch:
    ...
    def eval_agents(self, agents):
        ...
        )  #attention, don't deepcopy the problem instance, see Problem.py. Use problem_sampler if necessary instead.
        tt2 = time.time()
        elapsed = tt2 - tt1
        task_solvers = []
    ...
    def __call__(self,
        ...
        for it in range(iters):
            ...
            if hasattr(self.nov_estimator, "train"):
                self.nov_estimator.train(
                    np.random.choice(
                        pop, size=self.nov_estimator.growth_rate).tolist())
