...
class Agent(ABC):
    ...
    def to_batch_example(self):
        ...
        step_by_step_inputs = torch.cat([bd_conditioning, obs, action_offsets],
                                        1)

        return (text,
                step_by_step_inputs.reshape(
                    N * (bd_conditioning.shape[1] + obs.shape[1] +
                         action_offsets.shape[1])), action_cluster_idx
                )  #reshapes in a row major manner
