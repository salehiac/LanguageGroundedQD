...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if not self.flash:
            ...
            self.register_buffer(
                ...
                )  #### It is very important for it to be torch.tril, if it's upper triangular then QK^TV wont work
    ...
    def forward(self, x):
        ...
        if self.flash:
            ...
            assert T == self.bias.shape[-1] == self.bias.shape[
                -2], "this should not happend."  #this assert is used because of the previous line: why did the original code use :T, instead of ,:]?
...
def process_batch(batch,
    ...
    text_token_ids = tokenizer(
        text_batch,
        padding="max_length",
        max_length=max_len_pad,
        return_tensors="pt"
    ).input_ids  #padding might not be the most optimal way, but it simplifies things
    ...
    traj_batch = batch[1].reshape(
        BB, NN, DD).float()  #traj_batch[ex_i,j,:] is bd_j, obs_j, act_j
    ...
    jj = torch.multinomial(
        torch.ones_like(possible_js).float() / possible_js.shape[0], 1).item()
    ...
    for a_i in range(act_dims):
        ...
        centers_i = kmeans_lst[a_i].cluster_centers_[cluster_centers_ids[:, :,
                                                                         a_i]]
    ...
    act_tensor = subsequence[:, :, bd_dims + obs_dims:bd_dims + obs_dims +
                             act_dims] + cluster_center_coords
    ...
    if dbg:
        print(
            colored(
                f"[DBG] context_size={context_size}, T_text={T_text}, T_u={T_u}",
                "red",
                attrs=["bold"]))
        print(colored(f"[DBG] jj={jj}, T_u//3={T_u//3}", "red",
                      attrs=["bold"]))
        #print(batch[0])

    if input_normalizer is not None:
        bd_tensor, obs_tensor = input_normalizer(bd_tensor=bd_tensor,
...
class GPT_QDRL(nn.Module):
    def __init__(self, config: GPT_QDRLConfig):
        ...
        self.transformer = nn.ModuleDict(
            dict(
                word_token_embedding=nn.Embedding(config.vocab_size,
                                                  config.n_embd),
                word_pos_embedding=nn.Embedding(config.block_size,
                                                config.n_embd),
                timestamp_embedding=nn.Embedding(
                    1000, config.n_embd
                ),  #those are episode timesteps the 1000 here should be env.max_steps, TODO: don't hardcode that
                bd_embedding=RLMLP(config.n_bd_dims,
                                   h_sz=config.n_embd,
                                   out_sz=config.n_embd,
                                   dropout=False),
                obs_embedding=RLMLP(config.n_obs_dims,
                                    h_sz=config.n_embd,
                                    out_sz=config.n_embd,
                                    dropout=False),
                act_embedding=RLMLP(config.n_action_dims,
                                    h_sz=config.n_embd,
                                    out_sz=config.n_embd,
                                    dropout=False),
        ...
        for a_i in range(self.config.n_action_dims):
            self.action_cluster_heads.append(
                RLMLP(
                    in_sz=config.n_embd,
                    h_sz=32,
                    out_sz=config.kmeans_obj_lst[a_i].cluster_centers_.
                    shape[0],
                    scale_out_put=-1,
                    dropout=False
                )  #adding dropout to the last layer would be extremly dumb
            )

        self.num_clusters = np.prod(
            [x.cluster_centers_.shape[0] for x in self.config.kmeans_obj_lst])
        self.action_prediction_head_offsets = RLMLP(
            ...
            scale_out_put=-1,
            dropout=False
        )  #adding droupout to the last layer would be extremely dumb
    ...
    def reconfigure_heads(self, cfg):
        ...
        if cfg["cluster_idx_heads"]:
            ...
            for a_i in range(self.config.n_action_dims):
                self.action_cluster_heads.append(
                    RLMLP(in_sz=self.config.n_embd,
                          h_sz=cfg["cluster_idx_heads"],
                          out_sz=self.config.kmeans_obj_lst[a_i].
            ...
            print(
                colored(
                    "[WARNING] cluster_idx_heads were re-initialized with random weights, as specified in yaml config.",
                    "red",
                    attrs=["bold"]))
        ...
        if cfg["cluster_offset_head"]:
            ...
            print(
                colored(
                    "[WARNING] cluster_offset_head was re-initialized with random weights, as specified in yaml config.",
                    "red",
                    attrs=["bold"]))
    ...
    def forward(
        ...
        word_token_emb = self.transformer.word_token_embedding(
            word_idx)  # (B, T_text, embd_sz)
        word_pos_emb = self.transformer.word_pos_embedding(
            word_pos)  # (T_text, embd_sz)
        ...
        timestamp_rep = timestamp_tensor.reshape(-1, 1).repeat(1, 3).reshape(
            -1
        )  #each timestamp i is used for three consecutive tokens bd_i, obs_i, act_i (note that padding hasn't happened yet)
        timestamp_emb = self.transformer.timestamp_embedding(timestamp_rep)

        emb_sz = self.config.n_embd
        RL_emb = torch.cat([bd_emb, obs_emb, act_emb], -1).view(
            BB, 3 * LL, emb_sz
        )  #3*LL==3*(T_u//3), see the docstring of process_batch for T_u's definition

        RL_emb_dbg = torch.cat([bd_emb, obs_emb, act_emb],
                               -1)  #(BB, LL, n_embd*3)
        for ii in range(LL):
            assert (RL_emb_dbg[:, ii, 0:emb_sz] == RL_emb[:, ii * 3, :]).all()
            assert (RL_emb_dbg[:, ii, emb_sz:2 *
                               emb_sz] == RL_emb[:, ii * 3 + 1, :]).all()
            assert (RL_emb_dbg[:, ii, 2 * emb_sz:3 *
                               emb_sz] == RL_emb[:, ii * 3 + 2, :]).all()
        ...
        xx = torch.cat(
            [x1, x2], 1
        )  #(B,context_length-T_u%3,emb_sz), see the docstring of process_batch for T_u's defintion

        #padding if necessary
        num_pad = self.config.block_size - xx.shape[1]
        padding_tensor = torch.zeros(BB, self.config.block_size - xx.shape[1],
                                     emb_sz).to(xx.device)
        ...
        obs_inds = torch.arange(word_token_emb.shape[1] + 1,
                                self.config.block_size - num_pad,
                                step=3).to(xx.device)
        ...
        if not generation_mode:
            ...
            if loss_type == "multimodal_0":
                ...
                for a_i in range(self.config.n_action_dims):
                    ...
                    #### focal term i
                    predicted_actions_scores_i = self.action_cluster_heads[
                        a_i](zz)  #(BB,LL,num_clusters)
                    gamma = 2.0
                    proba_mat_i = torch.softmax(predicted_actions_scores_i,
                                                dim=-1)
                    p_t_i = proba_mat_i.gather(
                        2, cluster_centers_ids[:, :, [a_i]])
                    focal_vals_i = -(
                        (1 - p_t_i)**gamma) * torch.log(p_t_i + 1e-8)
                    term_1 += focal_vals_i.mean()

                    #bookkeeping for accuracy metric
                    argmax_act_i = predicted_actions_scores_i.argmax(
                        dim=-1).reshape(BB, LL, 1)
                    argmax_actions.append(argmax_act_i)

                term_1 /= float(self.config.n_action_dims)

                #### MSE term
                predicted_actions_offsets = self.action_prediction_head_offsets(
                    zz).reshape(BB, LL, self.num_clusters,
                                -1)  #(BB, LL, num_clusters, act_dims)
                ...
                term_2 = (
                    diff**2).mean()  #same as MSELoss with "mean" reduction
                ...
                argmax_actions = torch.cat(argmax_actions,
                                           2)  # (BB, LL, act_dims)
                ...
                for a_i in range(self.config.n_action_dims):
                    center_i = torch.Tensor(
                        self.config.kmeans_obj_lst[a_i].cluster_centers_[
                            argmax_actions[:, :,
                                           a_i].detach().cpu().numpy()]).to(
                                               UU.device)
            ...
            for a_i in range(self.config.n_action_dims):
                ...
                predicted_actions_scores_i = torch.softmax(
                    self.action_cluster_heads[a_i](zz) / temperature,
                    dim=-1)  #(BB, LL, num_clusters_i)

                #sample clusters
                if generation_strategy == "sample":
                    sampled_act_i = torch.multinomial(
                        predicted_actions_scores_i.reshape(
                            -1, self.config.kmeans_obj_lst[a_i].
                            cluster_centers_.shape[0]),
                        num_samples=BB)
                    sampled_act_i = sampled_act_i.reshape(BB, 1, 1)

                elif generation_strategy == "argmax":
                    sampled_act_i = predicted_actions_scores_i.argmax(
                        dim=-1).reshape(BB, 1, 1)
                elif generation_strategy == "nucleus":
                    decreasing_scores_idx = predicted_actions_scores_i.flatten(
                    ).argsort(descending=True).tolist()
                    target_mass = 0.95
                    cur_mass = 0
                    chosen_inds = []
                    for s_i in decreasing_scores_idx:
                        ...
                        if cur_mass >= target_mass:
                            break
                    #pdb.set_trace()
                    sampled_act_i = chosen_inds[torch.multinomial(
                        predicted_actions_scores_i[:, :,
                                                   chosen_inds].flatten() /
                        predicted_actions_scores_i.sum(),
                        num_samples=BB).item()]
                    sampled_act_i = torch.Tensor([sampled_act_i]).reshape(
                        BB, 1, 1).to(predicted_actions_scores_i.device).long()
                else:
            ...
            predicted_actions_offsets = self.action_prediction_head_offsets(
                zz).reshape(BB, 1, self.num_clusters,
                            -1)  #(BB, LL, num_clusters, act_dims)
            ...
            for a_i in range(self.config.n_action_dims):
                c_i = torch.Tensor(
                    self.config.kmeans_obj_lst[a_i].cluster_centers_[
                        sampled_acts[:, :, a_i].detach().cpu().numpy()]).to(
                            UU.device)
...
class QDRLPolicy:
    ...
    def __call__(self, obs):
        ...
        dummy_action = torch.zeros(
            1, self.cfg.n_action_dims
        )  #padding for compatibility, it has no effect on the prediction
        new_input = torch.cat(
            [self.target_bd,
             torch.tensor(obs).reshape(1, -1), dummy_action], 1).float()

        self.traj_window = torch.cat(
            [self.traj_window, new_input],
            0) if self.traj_window is not None else new_input  # (LL, DD)

        text_token_ids = self.tokenizer([self.textual_conditioning],
                                        padding="max_length",
                                        max_length=self.max_len_pad,
        ...
        obs_tensor = self.traj_window[:,
                                      self.cfg.n_bd_dims:self.cfg.n_bd_dims +
                                      self.cfg.n_obs_dims].unsqueeze(0)
        act_tensor = self.traj_window[:, self.cfg.n_bd_dims +
                                      self.cfg.n_obs_dims:self.cfg.n_bd_dims +
                                      self.cfg.n_obs_dims +
                                      self.cfg.n_action_dims].unsqueeze(0)
        traj_timestamps = torch.arange(self.traj_start_timestamp,
                                       self.traj_start_timestamp + LL)

        if self.input_normalizer is not None:
            bd_tensor, obs_tensor = self.input_normalizer(
                bd_tensor=bd_tensor, obs_tensor=obs_tensor)
            bd_tensor = bd_tensor.round(
                decimals=1)  #we do this in train, don't remove it here
